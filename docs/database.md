...
2. `save_embedding(student_id, embedding)` pickle embedding rồi lưu vào `embeddings`.
3. `get_all_embeddings()` tải tất cả embedding và giải mã bằng `pickle.loads()`.
...
8. `delete_student_and_embedding(student_id)` xóa sinh viên, embedding và lịch sử điểm danh liên quan.
...
## Ghi chú kỹ thuật
- Embedding khuôn mặt được lưu trong cột `embedding` dưới dạng BLOB.
- Backend dùng `pickle` để serialize/deserialize embedding khi lưu và đọc.
- Quan hệ giữa `students`, `embeddings`, `attendance` đảm bảo xóa dữ liệu liên quan khi xóa sinh viên.
